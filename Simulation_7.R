...
# I'm very curious, based on my simulation, what happen if I blind the algorithm? 
...
test_black_edu_mean = 4
test_hispanic_edu_mean = 4
test_asian_edu_mean = 4
test_white_edu_mean = 4

test_black_wh_mean = 4
test_hispanic_wh_mean = 4
test_asian_wh_mean = 4
test_white_wh_mean = 4

test_black_sl_mean = 4
test_hispanic_sl_mean = 4
test_asian_sl_mean = 4
test_white_sl_mean = 4


# Set up testing variance of education, work_history, skill level 
test_black_edu_var = 2
test_hispanic_edu_var = 2
test_asian_edu_var = 2
test_white_edu_var = 2

test_black_wh_var = 2
test_hispanic_wh_var = 2
test_asian_wh_var = 2
test_white_wh_var = 2

test_black_sl_var = 2
test_hispanic_sl_var = 2
test_asian_sl_var = 2
test_white_sl_var = 2
...
# Fit the logistic regression model NOTICE RACE IS BLIND 
logistic_model <- glm(hiring_outcome ~ education + work_history + skill_level, 
...
usl_pred_and_0bonus_table <- data.frame() # temporarily for debugging
...
for (i in 1: length(list_fix_data_pos_mino_high_1819)) {
  ...
  # Record the prediction and bonus - temporarily for debugging
  current_pred_and_0bonus_table <- current_round %>% 
    mutate(pred_and_0bonus = pred,
           RoundTime = i ) %>% 
    select(-education, -work_history, -skill_level, -hiring_outcome) %>% 
    rename(Race = race)
  usl_pred_and_0bonus_table <- rbind(usl_pred_and_0bonus_table, current_pred_and_0bonus_table)
  ...
  usl_base_line_model <- glm(hiring_outcome ~ education + work_history + skill_level, # BLIND RACE 
...
# Generate table of summary statistics (mean, median, sd) about the prediction of each race each round 
# After generate this summary table, use this to generate the following plot for median. 
summary_usl_pred_and_0bonus_table <- usl_pred_and_0bonus_table %>%
  group_by(Race, RoundTime) %>%
  summarize(
    mean = mean(pred_and_0bonus),
    median = median(pred_and_0bonus),
    sd = sd(pred_and_0bonus),
    .groups = "drop"
  ) 
race_pred_and_0bonus_plots <- ggplot(summary_usl_pred_and_0bonus_table, aes(x = RoundTime, y = median, color = Race)) +
  geom_line() +
  labs(title = "USL Median Prediction and 0Bonus by Round",
       x = "Round Number",
       y = "Median Prediction and 0Bonus") +
  theme_minimal()
print(race_pred_and_0bonus_plots)
...
ucb_pred_plus_bonus_table <- data.frame() # temporarily for debugging
...
for (i in 1: length(list_fix_data_pos_mino_high_1819)) {
  ...
  # Record the prediction plus bonus - temporarily for debugging
  current_pred_plus_bonus_table <- current_round %>% 
    mutate(pred_plus_bonus = pred_result$fit + pred_result$se.fit,
           RoundTime = i ) %>% 
    select(-education, -work_history, -skill_level, -hiring_outcome) %>% 
    rename(Race = race)
  ucb_pred_plus_bonus_table <- rbind(ucb_pred_plus_bonus_table, current_pred_plus_bonus_table)
  ...
  # Train with the new training data to get a new logistic model, which will be used for the next iteration
  ucb_base_line_model <- glm(hiring_outcome ~ education + work_history + skill_level, # BLIND RACE 
...
# After generate this summary table, use this to generate the following plot for median. 
# Temporarily for debugging. 
summary_ucb_pred_plus_bonus_table <- ucb_pred_plus_bonus_table %>%
  group_by(Race, RoundTime) %>%
  summarize(
    mean = mean(pred_plus_bonus),
    median = median(pred_plus_bonus),
    sd = sd(pred_plus_bonus),
    .groups = "drop"
  ) 
race_pred_plus_bonus_plots <- ggplot(summary_ucb_pred_plus_bonus_table, aes(x = RoundTime, y = median, color = Race)) +
  geom_line() +
  labs(title = "UCB Median Prediction plus Bonus by Round",
       x = "Round Number",
       y = "Median Prediction and Bonus") +
  theme_minimal()
print(race_pred_plus_bonus_plots)
